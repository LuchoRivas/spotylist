...
import { generateRandomString, SpotifyPlaylist, SpotifyPlaylistResponse } from "@spotylist/common";
...
app.get("/playlist", (req: any, res: any) => {
  try {
    const playlistId = "";
    const fetchPlaylist = () => {
      const options = {
        url: `${spotifyAPIBaseUrl}/playlists/${playlistId}`,
        headers: { Authorization: req.headers.authorization },
        json: true,
      };
